...
def valid_num(net, epoch):
    ...
    radio_conv = (np.arctan(opt.epochs - epoch) / np.pi)*0.1
    #print("arctan" , radio_conv)
        ...
        if isinstance(m1, nn.Conv2d):
            with torch.no_grad():
                ...
                if hight_fliter_count_m1 != num_filters:
                    low_cut = [i for i in low_fliter_index_per_m1 if i.abs() <= (low_fliter_index_per_m1[0].abs())*0.1]
