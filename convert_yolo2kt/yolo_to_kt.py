...
KT_CLASSES = ('Vechicle' , 'Pedestrian', 'TrafficLight' , 'TrafficSign') 

## converts the normalized positions  into integer positions
def unconvert(class_id, width, height, x, y, w, h, conf):
    ...
    return (class_id, xmin, xmax, ymin, ymax, conf)
...
def xml_transform(root, classes):  
    ...
    outpath = join(root, 'outputs', '%s_v001_1.xml')
    empty_cnt = 0
    ...
    for i in range(len(ids)):
        ...
        node_ver = SubElement(node_root, 'ver')
        node_ver.text = 'v001_1'
        
        node_folder = SubElement(node_root, 'folder')
        node_folder.text = img_id[0]
        img_name = img_id + '.jpg'
        ...
        node_segmented = SubElement(node_root, 'index_max')
        node_segmented.text = str(len(label_norm))
        ...
        if os.path.exists(target):
            ...
            for i in range(len(label_norm)):
                ...
                new_label = unconvert(labels_conv[0], width, height, labels_conv[1], labels_conv[2], labels_conv[3], labels_conv[4], labels_conv[5])
                ...
                node_conf.text = str(new_label[5])
                xml = tostring(node_root, pretty_print=True)  
                dom = parseString(xml)
